...
cp $current_dir/ms_install_5.9.1.tar.gz /home/midnightstreamer/ms_install.tar.gz
...
if echo "$(getconf GNU_LIBC_VERSION | cut -d' ' -f2) >= 2.30" | bc -l | grep -q 1 && [ -d /home/midnightstreamer/iptv_midnight_streamer/php_8.3 ]; then
PHPVER="8.3"
else
PHPVER="7.3"
fi
...
if [ "$PHPVER" = "8.3" ]; then
		rm -rf /home/midnightstreamer/iptv_midnight_streamer/php
		mv /home/midnightstreamer/iptv_midnight_streamer/php_8.3 /home/midnightstreamer/iptv_midnight_streamer/php

		rm -rf /home/midnightstreamer/iptv_midnight_streamer/wwwdir
		mv /home/midnightstreamer/iptv_midnight_streamer/wwwdir_8.3 /home/midnightstreamer/iptv_midnight_streamer/wwwdir

		mkdir /usr/local/openssl-3.0.14
		ln -s /usr/lib/ssl /usr/local/openssl-3.0.14/ssl
else
		rm -rf /home/midnightstreamer/iptv_midnight_streamer/php_8.3
		rm -rf /home/midnightstreamer/iptv_midnight_streamer/wwwdir_8.3
fi

echo "" > /home/midnightstreamer/iptv_midnight_streamer/config
...
if [ $p = "apt-get" ]; then
		...
		apt-get install python3-rebulk -y
		apt-get install python3-babelfish -y
		apt-get install python3-dateutil -y

		apt-get install python3-pip -y

		python3 -m pip install rebulk
		...
		yum install python3-guessit -y
		yum install python3-rebulk -y
		yum install python3-babelfish -y
...
if [ $p = "apt-get" ]; then
		...
		sh /home/midnightstreamer/iptv_midnight_streamer/NVIDIA-Linux-x86_64-570.144.run --silent --install-libglvnd
		rm /home/midnightstreamer/iptv_midnight_streamer/NVIDIA-Linux-x86_64-570.144.run
...
service mysql stop
																																																									 ...
																																																									 pkill -9 mysqld
																																																												 ...
																																																												 mysql -u root -p"iptv" -e "DROP DATABASE IF EXISTS midnight_iptv;CREATE DATABASE midnight_iptv CHARACTER SET utf8 COLLATE utf8_general_ci;DROP DATABASE IF EXISTS midnight_migrate;CREATE DATABASE midnight_migrate CHARACTER SET utf8 COLLATE utf8_general_ci;"
																																																									 			 ...
																																																									 			 mysql -u root -p"iptv" midnight_iptv -e "INSERT INTO servers (id, name, max_allowed_connections, guaranteed_speed, http_port, https_port, rtmp_port, server_ip, hostname, ssh_password, ssh_port, http_proxy_port, https_proxy_port, main, mysql_persistent_connections, fastcgi_services, ca, geoip_lb, isp_lb, do_update, disabled, version) VALUES (1, 'Main Server', 1000, 1000, '8000', '8001', '8002', '127.0.0.1', null, '', 22, 80, 443, 1, 0, 3, '$(cat /home/midnightstreamer/iptv_midnight_streamer/nginx/conf/server.crt)' , 0, 0, 0, 0, '5.9.1');
																																																									 																														 INSERT INTO syncs (server_id, name) VALUES (1, 'cronjobs');
																																																									 																														 INSERT INTO syncs (server_id, name) VALUES (1, 'blocked_ips');
																																																									 																														 INSERT INTO syncs (server_id, hash, name) VALUES (1, '', 'settings');
																																																									 																														 INSERT INTO syncs (server_id, hash, name) VALUES (1, '', 'config');
																																																									 																														 INSERT INTO administrators (username, password, email, language, modern_dashboard, anim) VALUES ('admin', '2cd71063c6a79832cdfeb889f4c072e425067fe2', 'example@domain.com', 'en', 1, 1);
...
if [ "$PHPVER" != "8.3" ]; then
		...
		rm /home/midnightstreamer/iptv_midnight_streamer/curl
fi

tee /etc/rsyslog.d/00-midnightstreamer.conf > /dev/null <<EOT 
cron.notice     										-/var/log/syslog
cron.*          										stop
:syslogtag, startswith, "tvheadend" stop
:syslogtag, startswith, "CRON" 			stop
:syslogtag, startswith, "sudo" 			stop
EOT
